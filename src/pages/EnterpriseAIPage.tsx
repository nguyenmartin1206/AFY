    ...
    <div className="space-y-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <h1 className="text-4xl md:text-5xl font-black text-text-primary tracking-tighter uppercase italic">
            Enterprise <span className="text-primary not-italic">AI</span>
          </h1>
          <p className="text-text-secondary mt-4 max-w-2xl text-sm font-medium leading-relaxed">
            High-scale corporate neural frameworks. Engineered for massive throughput, 
            security compliance, and cross-departmental synchronization.
          </p>
        </div>
        ...
        <div className="bg-primary/5 border border-primary/20 rounded-2xl p-6 backdrop-blur-sm self-start">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 bg-primary/10 rounded-xl flex items-center justify-center border border-primary/20">
              <span className="text-primary font-black text-xl">{enterpriseProducts.length}</span>
            </div>
            <div>
              <p className="text-[10px] font-black text-text-secondary uppercase tracking-widest">Active Units</p>
              <p className="text-xs font-bold text-text-primary uppercase italic">Enterprise Clusters</p>
            </div>
          </div>
        </div>
      </div>
          ...
          >
            ...
            <div className="p-8 flex flex-col flex-grow">
              ...
              <p className="text-xs text-text-secondary leading-relaxed mb-6 line-clamp-2 italic">
                Scalable infrastructure for {product.name.toLowerCase()} at organizational scale.
              </p>
      ...
      {enterpriseProducts.length === 0 && (
        <div className="text-center py-24 bg-bg-surface rounded-3xl border border-dashed border-border-main">
          <p className="text-text-secondary uppercase tracking-[0.3em] font-black text-sm">No enterprise modules detected</p>
        </div>
      )}
    </div>
  );
